...
function buildHrefToLabelLookup() {
  ...
  lookup.set("/closed_orders", "closed orders");
  ...
  lookup.set("/own-company-details", "own company details");
  lookup.set("/chart-of-accounts", "chart of accounts");
  lookup.set("/add-user", "users");
  lookup.set("/invoices-out", "invoices out");
  lookup.set("/crn-out", "credit notes out");
...
const hrefPermissionAliases = new Map([
  ...
  ["/chart-of-accounts", ["chart of accounts", "chart of account"]],
  ["/add-user", ["users", "user"]],
  ["/invoices-out", ["invoices out", "invoices-out", "invoice out", "invoice-out"]],
  [
    "/crn-out",
    [
      "credit notes out",
