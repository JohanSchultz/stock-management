export const menuTree = [
  {
    ...
    children: [
      ...
      {
        id: "own-company-details",
        label: "Own Company Details",
        href: "/own-company-details",
      },
      {
      ...
      { id: "add-user", label: "Users", href: "/add-user" },
      {
        id: "warehouse-space",
  ...
  },
  {
    ...
    children: [
      ...
      { id: "invoices-out", label: "Invoices Out", href: "/invoices-out" },
      {
        id: "crn-out",
...
function buildMenuHrefLookup() {
  ...
  lookup.set("closed orders", {
    id: "closed_orders",
    href: "/closed_orders",
    label: "Closed Orders",
  });
  lookup.set("stock taking", { id: "stock-taking", href: "/stock-taking", label: "Stock Taking" });
  lookup.set("invoices out", {
    id: "invoices-out",
    href: "/invoices-out",
    label: "Invoices Out",
  });
  lookup.set("invoice out", {
    id: "invoices-out",
    href: "/invoices-out",
    label: "Invoices Out",
  });
  lookup.set("credit notes out", {
    id: "crn-out",
    href: "/crn-out",
    label: "Credit Notes Out",
  });
  lookup.set("credit note out", {
    id: "crn-out",
    href: "/crn-out",
    label: "Credit Notes Out",
  });
  ...
  lookup.set("own company details", {
    id: "own-company-details",
    href: "/own-company-details",
    label: "Own Company Details",
  });
  lookup.set("chart of accounts", {
    id: "chart-of-accounts",
    href: "/chart-of-accounts",
    label: "Chart of Accounts",
  });
...
function resolveChildHref(row, match, displayLabel, rawLabel) {
  if (match?.href) return match.href;

  for (const value of [displayLabel, rawLabel, row.descr, row.menu_section]) {
    const key = normalizeLookupKey(value);
    if (!key) continue;
    const entry = menuHrefLookup.get(key);
...
function menuSectionMatchesParent(parent, menuSection) {
  ...
  if (parent.id === "function") {
    ...
    if (
      ...
      sectionLower === "closed orders" ||
      sectionLower === "invoice out" ||
      sectionLower === "invoices out" ||
      sectionLower === "credit notes out" ||
      sectionLower === "credit note out" ||
      sectionLower === "credit notes-out" ||
      sectionLower === "credit note-out" ||
      sectionLower === "crn out"
    ) {
      return true;
    }
  }
...
function normalizePermissionRow(row) {
  ...
  if (sectionLower === "customer" || sectionLower === "customers") {
    return {
      menu_section: "Administration",
      descr: descr || "Customers",
    };
  }

  if (sectionLower === "own company details") {
    return {
      menu_section: "Administration",
      descr: descr || "Own Company Details",
    };
  }

  if (sectionLower === "chart of accounts") {
    return {
      menu_section: "Administration",
      descr: descr || "Chart of Accounts",
    };
  }

  if (sectionLower === "users" || sectionLower === "user") {
    return {
      menu_section: "Administration",
      descr: descr || "Users",
    };
  ...
  if (sectionLower === "invoice out" || sectionLower === "invoices out") {
    return {
      menu_section: "Functions",
      descr: descr || "Invoices Out",
    };
  }
  ...
  return menuDynamicTree.map((parent) => {
    ...
    const children = rows
      ...
      .map((row, index) => {
        ...
        const href = resolveChildHref(row, match, displayLabel, rawLabel);
