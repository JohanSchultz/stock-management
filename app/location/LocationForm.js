...
function normalizeLocations(data) {
  ...
  return data.map((row, index) => ({
    ...
    rowKey:
      row.id != null
        ? `location-${row.id}`
        : `location-row-${index}-${row.descr ?? "unknown"}`,
  }));
}
...
export function LocationForm() {
  ...
  const loadLocations = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc("pr_locations");
  ...
  async function handleSave() {
    ...
    try {
      const supabase = createClient();
      const { error: rpcError } = await supabase.rpc("pi_locations", {
        p_locations: locationName,
      });
  ...
  function handleRowClick(row) {
    ...
    setLocationId(row.id != null ? String(row.id) : "");
  ...
  async function handleChange() {
    ...
    if (!locationName || Number.isNaN(id)) return;
    ...
    try {
      const supabase = createClient();
      const { error: rpcError } = await supabase.rpc("pu_locations", {
        p_id: id,
        p_locations: locationName,
        p_is_active: active,
      });
      ...
      await refreshAfterAction("Location updated.");
    } catch (err) {
  ...
  async function handleDeactivate() {
    ...
    try {
      const supabase = createClient();
      const { error: rpcError } = await supabase.rpc("pd_locations", {
        p_id: id,
      });

      if (rpcError) throw rpcError;

      await refreshAfterAction("Location deactivated.");
    } catch (err) {
        ...
        {editMode && (
          <>
            ...
            <button
              ...
              disabled={loading || !location.trim() || !locationId}
