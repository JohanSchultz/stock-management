...
function normalizeReturnReasons(data) {
  if (!Array.isArray(data)) return [];

  return data.map((row, index) => ({
    ...row,
    id: row.id ?? row.return_reason_id ?? null,
    descr: row.descr ?? "",
    is_active: row.is_active,
    rowKey:
      row.id != null
        ? `return-reason-${row.id}`
        : `return-reason-row-${index}-${row.descr ?? "unknown"}`,
  }));
}
...
export function ReturnReasonForm() {
  ...
  const loadReturnReasons = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc("pr_return_reasons");
      if (rpcError) throw rpcError;
      setReturnReasons(normalizeReturnReasons(data));
    } catch (err) {
  ...
  async function handleSave() {
    const returnReasonName = returnReason.trim();
    if (!returnReasonName) return;
    ...
    try {
      const supabase = createClient();
      const { error: rpcError } = await supabase.rpc("pi_return_reasons", {
        p_return_reasons: returnReasonName,
      });

      if (rpcError) throw rpcError;

      setSuccess("Return reason saved.");
      initializeForm();
      await loadReturnReasons();
    } catch (err) {
      setError(err.message ?? "Failed to save return reason");
    } finally {
      setLoading(false);
    }
  }
  ...
  function handleRowClick(row) {
    ...
    setReturnReasonId(row.id != null ? String(row.id) : "");
  ...
  async function handleChange() {
    const returnReasonName = returnReason.trim();
    const id = Number.parseInt(returnReasonId, 10);
    if (!returnReasonName || Number.isNaN(id)) return;
    ...
    try {
      const supabase = createClient();
      const { error: rpcError } = await supabase.rpc("pu_return_reasons", {
        p_id: id,
        p_return_reasons: returnReasonName,
        p_is_active: active,
      });
      ...
      await refreshAfterAction("Return reason updated.");
    } catch (err) {
  ...
  async function handleDeactivate() {
    ...
    try {
      const supabase = createClient();
      const { error: rpcError } = await supabase.rpc("pd_return_reasons", {
        p_id: id,
      });

      if (rpcError) throw rpcError;

      await refreshAfterAction("Return reason deactivated.");
    } catch (err) {
      setError(err.message ?? "Failed to deactivate return reason");
    } finally {
      setLoading(false);
    }
  }
        ...
        {editMode && (
          <>
            ...
            <button
              ...
              disabled={loading || !returnReason.trim() || !returnReasonId}
            ...
            <button
              ...
              disabled={loading || !returnReasonId}
